    ...
    def transform(self) -> TransformResult:
        ...
        try:
            tickets_dataframe = (
                ...
                .withColumn(
                    "class",
                    F.coalesce(F.lower(F.trim(F.col("class"))), F.lit("regular")),
                )
                ...
                .withColumn(
                    "has_child",
                    F.coalesce(
                        F.col("extra_info.child_discount").cast(BooleanType()),
                        F.lit(False),
                    ),
                )
                .withColumn(
                    "family_flag",
                    F.when(
                        F.col("extra_info.family_members") > 1, F.lit(True)
                ...
                .withColumn(
                    "booking_lead_days",
                    F.greatest(
                        F.datediff(F.col("departure_date"), F.col("created_at")),
                        F.lit(0),
                    ),
                )
            )

            window_latest = Window.partitionBy("ticket_id").orderBy(
                F.col("created_at").desc()
            )
            window_timestamp = Window.partitionBy("ticket_id")

            tickets_deduped = (
                tickets_dataframe.withColumn("rn", F.row_number().over(window_latest))
                .withColumn("active_status", F.first("status").over(window_latest))
                .withColumn(
                    "paid_at",
                    F.max(F.when(F.col("status") == "paid", F.col("created_at"))).over(
                        window_timestamp
                    ),
                )
                .withColumn(
                    "cancelled_at",
                    F.max(
                        F.when(F.col("status") == "cancelled", F.col("created_at"))
                    ).over(window_timestamp),
                )
                .withColumn(
                    "refunded_at",
                    F.max(
                        F.when(F.col("status") == "refunded", F.col("created_at"))
                    ).over(window_timestamp),
                )
                .filter(F.col("rn") == 1)
                .alias("td")
