    ...
    def transform(self):
        try:
            ...
            tickets_dataframe = tickets_dataframe.withColumn(
                "refund_date", F.to_date("refunded_at")
            )

            refund_loss_dataframe = (
                tickets_dataframe.filter(F.col("refunded_at").isNotNull())
            ...
            result_df = (
                ...
                .withColumn(
                    ...
                    F.round(F.col("avg_days_cancel_to_refund"), 2),
                )
                .withColumn(
                    "avg_hours_to_refund", F.round(F.col("avg_hours_to_refund"), 2)
                )
                .withColumn(
                    "avg_days_created_to_refund",
                    F.round(F.col("avg_days_created_to_refund"), 2),
                )
